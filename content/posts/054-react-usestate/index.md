...
`<Bulb bulbSwitch={true|false} />` is a stateless functional component that accepts a boolean prop `isOn`. The prop `isOn` indicates whether the bulb is lighted (`true`) or not (`false`).  
...
function Bulb() {
  const [bulbSwitch] = useState(false);
  // ...
}
```

What's important about `bulbSwitch` state variable is that unless the state is not changed, it will remains the same even if the React component rerenders.  

The whole idea of state is to be able to change it. Let's see how to do this in a final step.  

## 1.4 Updating state

As you know already `useState(false)` returns an array of 2 items, where the first item is the current state. Now the second item of the returned array is a function that accepts one argument: the new state value. Use this function to update the state.   
