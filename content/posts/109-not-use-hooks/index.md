...
React hooks make side-effects and state management easier. I've been enjoying working with hooks since the beginning.  

However, despite hooks simplicity, there are situations when you just can't understand why they don't work as you expect.  

In this post, I will describe situations of how *Not* to use React hooks, or the common anti-patterns.  
...
Unfortunately, even if the `increase()` is called 3 times inside the `handleClick()`, when the button is clicked the `count` state variable is increased only by 1. Hm...
...
A state variable `isFirst` holds the information whether now is the first render. That's a problem &mdash; as soon as you update `setIsFirst(false)` you trigger another re-render: and for no reason.  

Holding the information whether this is the first render of the component shouldn't be stored in the state. Such information I call the infrastructure info: the one that helps the component to render, however, the infrastructure information isn't rendered by itself as an output.  
...
`isFirstRef` is a reference that holds the information if this the first render of the component. `isFirstRef.current` property is used to access and update the value of the reference.  

What's important updating a reference `isFirstRef.current = false` doesn't trigger a re-render.  

(Add more examples from the real world regarding roads, traffic lights)
...
A lot of side-effects, like making a fetch request, setting a timeout using `setTimeout()` are asynchronous.  
...
However, if you've been working for a while with React hooks, you might encounter situations when you just can't understand why they behave differently than you expect.  

That's because knowing how to use React hooks is not enough: you also should know how Not to use them.  

In this post, I've presented the common situations that you shouldn't do with hooks.  
...
The second thing you should avoid is creating and using stale state values. That often could happen if you calculate the state using previous state values. To solve that simply use a functional way to update the state. Also, don't forget to indicate correctly the dependencies array for hooks that accept callbacks as arguments: e.g. `useEffect(callback, deps)`, `useCallback(callback, deps)`.  
