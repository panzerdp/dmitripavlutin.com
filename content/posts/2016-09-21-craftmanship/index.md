...
This excuse is a wrong approach if you are determined to be a successful software developer. To build complex and top quality applications you must have algorithms knowledge.  

Indeed algorithms are implemented in standard libraries and developer doesn't need manually to write them. But the lack of expertise doesn't allow developer to see when a problem is elegantly solved using a specific algorithm.  
...
Every programmer started at this level, including myself.  

A beginner developer got really excited by a particular framework, ReactJS for example. He followed some framework tutorials  and has a decent level of its understanding.  
...
When a part of the application may benefit from a well-known algorithm, developer doesn't see that and starts inventing the wheel. And the invented wheel is usually low-quality and time waster.  
...
Proficient developer has mastered well ReactJS framework and JavaScript language. The number of hacks in his code are minimal.  

Proficient developer can see the situations when certain data structure or algorithm makes the code more productive. But only until a limited level.

For example an application has a component that searches often in an array with lots of elements. If the array is altered rarely, the developer concludes that its reasonable to sort the array. And when a search operation is necessary, use the [binary search algorithm](https://en.wikipedia.org/wiki/Binary_search_algorithm), which is very efficient in this situation.  

At rectangle case developer is able to create quality applications, but not the most efficient. 
...
Despite the fact that fundamentals are difficult, there is a pleasant bonus: it doesn't change often. Starting 80's or even 70's weren't created any significant new algorithms or data structures. Learn them once and good, then repeat them time to time, and you're good to go.  

### The interview process

Big software companies like Google, Apple and Microsoft are serious about algorithms and data structures. An interview at these companies contains many computer science questions, white board coding, balancing trees and so on.  

I've read many posts that criticize such approach that companies query the wrong skills. They say that developer doesn't deal with sorting algorithms or balancing trees in "real world" programming.  
...
I personally agree with computer science interview questions. Because the software development industry is moving so fast, company needs developers that have a solid background.  

On one hand such developer can efficiently solve complicated problems. On the other hand he can easily move forward in new languages and frameworks, adapting to fast evolving industry.  
...
There is no shortcut to become a master. What makes you better is **learning** and **practicing**. 
...
After solidifying your ground in those directions, take a next level:
